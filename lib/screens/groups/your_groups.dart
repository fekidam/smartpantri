...
class _YourGroupsScreenState extends State<YourGroupsScreen> {
    ...
    for (var gid in ids) {
      ...
      if (doc.exists && doc.data()!.containsKey('items')) {
        ...
        for (var raw in items) {
          if (raw['isPurchased'] == true && raw['selectedBy'] == currentEmail) {
  ...
  Future<void> _togglePurchased(Map<String, dynamic> item) async {
    ...
    final was = item['isPurchased'] as bool? ?? false;
    final now = !was;
    setState(() {
      item['isPurchased'] = now;
      if (now) item['selectedBy'] = user.email;
      else item.remove('selectedBy');
    });
    ...
    if (now) {
      if (uidx != -1) uItems[uidx] = item;
      else uItems.add(item);
    } else {
  ...
  Widget build(BuildContext context) {
    ...
    final l10n = AppLocalizations.of(context)!;
    final items = widget.isGuest ? _guestItems : _userItems;
    final locale = Localizations.localeOf(context).languageCode;
        ...
        child: SafeArea(
          child: items.isEmpty
            ...
            itemBuilder: (_, i) {
              ...
              final convertedPrice = locale == 'hu' ? (double.tryParse(price) ?? 0.0) * usdToHufRate : double.tryParse(price) ?? 0.0;
              ...
              return Card(
                ...
                child: ListTile(
                  title: Text(
                    name,
                    style: TextStyle(
                      color: checked ? Theme.of(context).disabledColor : Theme.of(context).colorScheme.onSurface,
                  ...
                  subtitle: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${l10n.priceLabel}: ${convertedPrice.toStringAsFixed(2)} $currencySymbol',
                        style: TextStyle(
                          color: Theme.of(context).colorScheme.onSurface.withOpacity(0.7),
                          fontSize: 14 * fontSizeScale,
                        ),
